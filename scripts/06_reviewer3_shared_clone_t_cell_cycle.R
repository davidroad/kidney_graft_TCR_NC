...
internal_patients <- sort(unique(as.character(cd8$patient)))
patient_map <- setNames(sprintf("P%d", seq_along(internal_patients)), internal_patients)
cd8$patient <- unname(patient_map[as.character(cd8$patient)])
batch_public <- as.character(cd8$batch)
for (id in names(patient_map)) batch_public <- sub(id, patient_map[[id]], batch_public, fixed = TRUE)
cd8$batch <- batch_public
...
gene_table <- bind_rows(
  ...
  mutate(available_in_shared_t = gene %in% rownames(shared_cd8))
...
for (rds_dir in rds_dirs) {
  saveRDS(shared_cd8, file.path(rds_dir, "reviewer3_shared_clone_t_cell_cycle_scored.rds"))
}
...
message("Shared-clonotype T cells: ", ncol(shared_cd8))
...
feature_grid_genes <- intersect(c("MKI67", "CDKN2A"), rownames(shared_cd8))
