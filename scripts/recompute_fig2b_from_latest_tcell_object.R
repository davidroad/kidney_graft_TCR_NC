...
# prior across clusters. Bound proportions before the logit to handle zeros.
...
for (i in seq_len(nrow(sample_meta))) {
  ...
  p <- pmin(pmax(z$proportion_pct / 100, 0.005), 0.995)
  response[, i] <- qlogis(p)
}
...
stats <- data.frame(
  ...
  model = paste(
    ...
    "design ~ patient + condition; bounded logit proportion; BH across clusters"
  ),
