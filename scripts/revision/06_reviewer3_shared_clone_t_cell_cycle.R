...
all_clone_file <- arg_value(
  "--input",
  Sys.getenv(
    "SHARED_CLONE_INPUT_RDATA",
    file.path(repo_dir, "data", "processed", "all_clone_filter_10_28.Rdata")
  )
)
if (!file.exists(all_clone_file)) {
  stop(
    "Shared-clonotype T-cell input object not found: ", all_clone_file, "\n",
    "Provide it with --input /path/to/all_clone_filter_10_28.Rdata or SHARED_CLONE_INPUT_RDATA."
  )
}

out_root <- arg_value("--out", file.path(repo_dir, "results", "shared_clone_t_cell_cycle"))
figure_dirs <- file.path(out_root, "figures")
table_dirs <- file.path(out_root, "tables")
rds_dirs <- file.path(out_root, "rds")
invisible(lapply(c(figure_dirs, table_dirs, rds_dirs), dir.create, recursive = TRUE, showWarnings = FALSE))
...
t_cells <- load_env[["all_clone_test_filter"]]
DefaultAssay(t_cells) <- "RNA"

required_meta <- c("patient", "label", "batch", "t_cdr3s_aa", "cell_type_annotated_granularity")
missing_meta <- setdiff(required_meta, colnames(t_cells@meta.data))
if (length(missing_meta)) {
  stop("Input object is missing required metadata: ", paste(missing_meta, collapse = ", "))
}

internal_patients <- sort(unique(as.character(t_cells$patient)))
patient_map <- setNames(sprintf("P%d", seq_along(internal_patients)), internal_patients)
t_cells$patient <- unname(patient_map[as.character(t_cells$patient)])
batch_public <- as.character(t_cells$batch)
for (id in names(patient_map)) batch_public <- sub(id, patient_map[[id]], batch_public, fixed = TRUE)
t_cells$batch <- batch_public
patients <- unname(patient_map)
shared_t <- t_cells
shared_summary <- t_cells@meta.data %>% count(patient, name = "shared_cell_count") %>% mutate(shared_clone_count = NA_integer_)
DefaultAssay(shared_t) <- "RNA"
shared_t$label <- factor(as.character(shared_t$label), levels = c("PBMC", "Graft"))
shared_t$patient <- factor(as.character(shared_t$patient), levels = patients)
shared_t$cell_type_annotated_granularity <- factor(as.character(shared_t$cell_type_annotated_granularity))
...
gene_table <- bind_rows(
  ...
  tibble(gene_group = "T-cell state context", gene = context_genes)
) %>%
  distinct(gene_group, gene) %>%
  mutate(available_in_shared_t = gene %in% rownames(shared_t))
write_all_tables(gene_table, "reviewer3_shared_clone_t_cell_cycle_gene_set.csv")

proliferation_present <- intersect(proliferation_genes, rownames(shared_t))
arrest_present <- intersect(arrest_genes, rownames(shared_t))
survival_death_present <- intersect(survival_death_genes, rownames(shared_t))
s_genes_present <- intersect(cc.genes.updated.2019$s.genes, rownames(shared_t))
g2m_genes_present <- intersect(cc.genes.updated.2019$g2m.genes, rownames(shared_t))

shared_t <- AddModuleScore(shared_t, features = list(proliferation_present), name = "Proliferation_module", assay = "RNA", seed = 1)
shared_t <- AddModuleScore(shared_t, features = list(arrest_present), name = "Cell_cycle_arrest_module", assay = "RNA", seed = 1)
shared_t <- AddModuleScore(shared_t, features = list(survival_death_present), name = "Survival_apoptosis_module", assay = "RNA", seed = 1)
shared_t <- CellCycleScoring(
  shared_t,
...
expr_genes <- intersect(c("CXCR6", proliferation_present, arrest_present, survival_death_present, context_genes), rownames(shared_t))
expr <- FetchData(shared_t, vars = expr_genes)
shared_t$CXCR6_status <- factor(if_else(expr$CXCR6 > 0, "CXCR6+", "CXCR6-"), levels = c("CXCR6-", "CXCR6+"))
shared_t$MKI67_positive <- expr$MKI67 > 0
shared_t$proliferation_gene_positive <- rowSums(expr[, proliferation_present, drop = FALSE] > 0) > 0
shared_t$label_cxcr6 <- factor(
  paste(shared_t$label, shared_t$CXCR6_status, sep = " "),
  levels = c("PBMC CXCR6-", "PBMC CXCR6+", "Graft CXCR6-", "Graft CXCR6+")
)

for (rds_dir in rds_dirs) {
  saveRDS(shared_t, file.path(rds_dir, "reviewer3_shared_clone_t_cell_cycle_scored.rds"))
}

cell_counts <- shared_t@meta.data %>%
  count(patient, label, CXCR6_status, name = "cell_count") %>%
  arrange(patient, label, CXCR6_status)
write_all_tables(shared_summary, "reviewer3_shared_clone_t_shared_clone_counts.csv")
write_all_tables(cell_counts, "reviewer3_shared_clone_t_cxcr6_cell_counts.csv")

expr_tbl <- expr %>%
  as_tibble(rownames = "cell")
cell_meta <- shared_t@meta.data %>%
  mutate(cell = rownames(shared_t@meta.data)) %>%
...
patient_summary <- cell_meta %>%
  ...
  arrange(patient, label, CXCR6_status)
write_all_tables(patient_summary, "reviewer3_shared_clone_t_cell_cycle_patient_summary.csv")
...
write_all_tables(patient_stats, "reviewer3_shared_clone_t_cell_cycle_patient_level_stats.csv")
...
write_all_tables(cell_stats, "reviewer3_shared_clone_t_cell_cycle_cell_level_stats_exploratory.csv")

message("Shared-clonotype T cells: ", ncol(shared_t))
...
lims <- square_umap_limits(shared_t, "umap")
p_label <- DimPlot(shared_t, reduction = "umap", group.by = "label", cols = label_cols, pt.size = 0.28, label = FALSE) %>%
  style_feature(lims, "Tissue")
p_state <- DimPlot(shared_t, reduction = "umap", group.by = "cell_type_annotated_granularity", pt.size = 0.28, label = FALSE) %>%
  style_feature(lims, "T-cell state")
p_cx_status <- DimPlot(shared_t, reduction = "umap", group.by = "CXCR6_status", cols = cxcr6_cols, pt.size = 0.28, label = FALSE) %>%
  style_feature(lims, "CXCR6 status")

feature_summary <- FeaturePlot(
  shared_t,
  reduction = "umap",
  features = intersect(c("CXCR6", "MKI67", "TOP2A", "PCNA"), rownames(shared_t)),
  split.by = "label",
...
phase_summary <- patient_summary %>%
  ...
  mutate(
    ...
    label_cxcr6 = factor(paste(label, CXCR6_status), levels = levels(shared_t$label_cxcr6))
  ) %>%
...
save_all_fig(
  summary_fig,
  "12_reviewer3_shared_clone_t_cell_cycle_summary.pdf",
  width = 14.4,
  height = 12.6,
  png = TRUE
)

feature_grid_genes <- intersect(c("MKI67", "CDKN2A"), rownames(shared_t))
if (length(feature_grid_genes) != 2L) stop("MKI67 and CDKN2A are required for the Fig. 2G-style feature plots.")
feature_grid_fig <- FeaturePlot(
  shared_t,
  reduction = "umap",
  features = feature_grid_genes,
  split.by = "label",
  cols = c("lightgrey", "firebrick3"),
  ncol = 2,
  label = FALSE,
  ...
  plot_annotation(tag_levels = "A", title = "Shared-clonotype T-cell marker feature plots for cell-cycle and death-related genes")
save_all_fig(
  feature_grid_fig,
  "13_reviewer3_shared_clone_t_cell_cycle_marker_featureplots.pdf",
  width = 8.0,
  height = 8.0,
  png = TRUE
)
...
violin_genes <- gene_table %>%
  filter(available_in_shared_t, gene_group != "T-cell state context") %>%
  mutate(gene = factor(gene, levels = unique(gene))) %>%
  pull(gene) %>%
  as.character()
violin_df <- FetchData(shared_t, vars = violin_genes) %>%
  ...
  mutate(
    ...
    label_cxcr6 = factor(label_cxcr6, levels = levels(shared_t$label_cxcr6))
  )
...
violin_fig <- ggplot(violin_df, aes(label_cxcr6, expression, fill = label_cxcr6)) +
  ...
  labs(
    ...
    fill = NULL,
    title = "Shared-clonotype T-cell marker violin plots by tissue and CXCR6 status"
  ) +
  theme_nc(8) +
  theme(
...
save_all_fig(
  violin_fig,
  "14_reviewer3_shared_clone_t_cell_cycle_marker_violins.pdf",
  width = 16.0,
...
methods_text <- paste(
  ...
  "",
  "The saved Fig. 2D shared-clonotype object contains 4,110 T cells whose TCR CDR3 amino-acid clonotypes were detected in both graft and PBMC compartments within the same patient. The object includes CD8, CD4, and Treg annotations, consistent with the manuscript's shared-clonotype T-cell definition. CXCR6-positive cells were defined by detectable CXCR6 RNA expression (>0 log-normalized expression, equivalent to non-zero normalized signal).",
  "",
  ...
  "",
  "Main interpretation: CXCR6+ shared-clonotype T cells were predominantly non-cycling by MKI67 and Seurat cell-cycle scoring. The MKI67-positive fraction was low in both graft and PBMC compartments. These results provide an in situ assessment of the full Fig. 2D shared-clonotype T-cell population and are not a replacement for the ex vivo proliferation assay. The annotated CXCR6+ effector CD8 states may support the manuscript's CD8-focused biological interpretation, while the 4,110-cell population-level metrics remain identified as shared-clonotype T-cell metrics.",
  sep = "\n"
)
