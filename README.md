...
| Supplementary Fig. 1-2 | Per-sample all-CD45 and T-cell UMAP projections and cluster marker summaries | `scripts/reproducibility/04_generate_suppfig1_2_from_objects.R` |
| Supplementary Fig. 3-4 | T-cell clone-size distributions, paired-sample overlap, and patient-level clonotype expansion | `scripts/reproducibility/05_generate_suppfig3_4_from_tcr_object.R` |
| Supplementary Fig. 5 | Shared-clonotype stem-like, terminal-differentiation, cytotoxic, MKI67, and CDKN2A expression | `scripts/reproducibility/06_generate_suppfig5_from_shared_object.R` |
...
- **Supplementary Fig. 1-5 panel generation:** `04_generate_suppfig1_2_from_objects.R`, `05_generate_suppfig3_4_from_tcr_object.R`, and `06_generate_suppfig5_from_shared_object.R` generate each released panel directly from the processed single-cell and TCR objects.
...
- Supplementary Figs. 1-2 use the integrated all-CD45 and T-cell objects. Supplementary Figs. 3-4 use the T-cell object and its paired-chain clonotype list. Supplementary Fig. 5 uses the shared-clonotype object, the CD8 T-cell object, and the panel-E cell-selection table supplied in Figshare.
...
Software dependencies are listed in `environment.yml`. Install `scRepertoire` from the matching Bioconductor release if it is not already present. The released figures, metrics, and tables are available at https://doi.org/10.6084/m9.figshare.33135038.v3.
...
- Figshare source-data record: https://doi.org/10.6084/m9.figshare.33135038.v3
